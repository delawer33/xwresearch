...
| `mawtarx-connect` (`exonware.mawtarx_connect`) | Marketplace scrapers (~660 registered, ~70 wired adapters) — the **library**. The sweep engine (`runner.py:SweepRunner`) runs as a **standalone systemd process**, not in any API. | **library + live crawler** — see the `mawtarx-scraper-runner` row below |
| `mawtarx-connect-api`                          | HTTP surface for mawtarx-connect — port **8253**. A `karaa-connect-api` upstream. Plain `APIRouter`. **Read/debug only — it does NOT run scrapes** (no scheduler in its lifespan). | **LIVE service** |
| **`mawtarx-scraper-runner`** (systemd unit, no repo of its own) | **The process that actually scrapes on the VPS.** Console script `mawtarx-runner` → `exonware.mawtarx_connect.runner:main` → `SweepRunner.loop`. Own user, opens no DB, POSTs sweeps to mawtarx-api over HTTP with a `listings.write` token. Deliberately a separate process so scraping never competes with mawtarx-api's event loop. Unit file: `repos/mawtarx-connect/deploy/mawtarx-scraper-runner.service`. | **LIVE on the VPS** (confirmed 2026-07-23) |
