...
| `markibx` (`exonware.markibx`)                 | Car knowledge base (specs/catalog); used in-process by mawtarx. Its **only** catalog is the **spine**: a canonical `make → model → generation` registry (`spine.py`) built from committed seed files on every boot (`src/exonware/markibx/data/spine_seed/` — packaged, **not** a repo-root `data/`; 100 makes / 4,924 models / 5,256 generations, `markibx depth` 2026-08-04), each fact tagged `{value, source, confidence}`; resolve via `resolve`/`resolve_sheet`, walk via `browse_*`. The legacy flat `CatalogVehicle`/`catalog_key`/`ICatalogStore` surface was **deleted** (markibx `docs/adr/0003`, superseding D-011; implemented across all repos 2026-07-29). **Deployed to the dev VPS 2026-07-29** (coordinated 6-repo deploy). Membership then **widened live from Wikidata (#25, deployed 2026-07-30)** and grown again by the 2026-08-03 structural-soundness pass: 249 → **5,256 gens / 4,924 models / 100 makes** (the 2026-08-04 `baw`→`faw` rekey split one mislabelled make into two, ADR 0016 — **deployed**), nearly all identity-only shells (**7% spec depth** — specs are a separate pass; `markibx depth`'s wider `curated` count includes lineage-only gens and is not depth, see that repo's `CONTEXT.md`). Curated models kept intact — widening only creates the uncurated tail (curated-gate). | **load-bearing** (via mawtarx)                          |
...
| A trustable spec sheet (provenance/confidence) | `repos/markibx`'s **spine** — `GET /catalog/resolve` + `/catalog/browse/*`, now the **only** catalog (the flat `/catalog/car*`/`/catalog/vehicles` surface was deleted, ADR 0003). 5,256 generations / 4,924 models / 100 makes (breadth-widened from Wikidata #25, deployed 2026-07-30; grown 2026-08-03, rekeyed + redeployed 2026-08-04) — but **7% spec depth**: 367 gens carry a real spec, and that spec is US/CA-layer (EPA/NHTSA), with the `GCC` layer at 1 gen. **Hard break DEPLOYED to dev VPS 2026-07-29.** Global-first: membership is what was manufactured, not what mawtarx has listed. Decisions: `repos/markibx/docs/adr/` (0001–0008) + [`docs/markibx-mvp-catalog-model.md`](docs/markibx-mvp-catalog-model.md) for the superseded GCC framing |
